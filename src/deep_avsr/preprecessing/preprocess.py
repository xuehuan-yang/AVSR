...
def noise_generate(filesList, noise_second):
    ...
    # k1,k2,k3,k4,k5 = 16000, 3600, 60,20, 32727  # default
    k1, k2, k3, k4, k5 = 160, noise_second, 60, 20, 32727

    noise = np.empty((0))
    while len(noise) < k1 * k2:
        noisePart = np.zeros(k1 * k3)
        indices = np.random.randint(0, len(filesList), k4)
        for ix in indices:
    ...
    wavfile.write(args["DATA_DIRECTORY"] + "/noise.wav", k1, noise)
